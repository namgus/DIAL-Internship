...
class Ranker():
    ...
    def rank(self, Q_128, Q_V, pids=None):
        pids = self.retrieve(Q_128, verbose=False)[0] if pids is None else pids

        assert type(pids) in [list, tuple], type(pids)
        assert Q_128.size(0) == 1, (len(pids), Q.size())
        assert all(type(pid) is int for pid in pids)

        scores = []
        if len(pids) > 0:
            Q_128 = Q_128.permute(0, 2, 1)
            scores = self.index.rank(Q_128, Q_V, pids)
